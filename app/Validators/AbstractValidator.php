...
class AbstractValidator {
    ...
    protected function required(mixed $value, string $field): void {
        if (empty($value)) {
            $this->validationObject->addError($field, "This field is required.");
        }
    }

    protected function minLength(mixed $value, string $field, int $minLength): void {
        if (strlen($value) < $minLength) {
            $this->validationObject->addError($field, "Must be at least $minLength characters.");
        }
    }

    protected function maxLength(mixed $value, string $field, int $maxLength): void {
        if (strlen($value) > $maxLength) {
            $this->validationObject->addError($field, "Must be more than $field characters.");
        }
    }

    protected function uppercase(mixed $value, string $field): void {
        if (!preg_match("/[A-Z]/", $value)) {
            $this->validationObject->addError($field, "This field requires at least one uppercase letter.");
        }
    }

    protected function lowercase(mixed $value, string $field): void {
        if (!preg_match("/[a-z]/", $value)) {
            $this->validationObject->addError($field, "This field requires at least one lowercase letter.");
        }
    }

    protected function number(mixed $value, string $field): void {
        if (!preg_match("/\d/", $value)) {
            $this->validationObject->addError($field, "This field requires at least one number.");
        }
    }

    protected function specialchar(mixed $value, string $field): void {
        if (!preg_match('/^[a-zA-Z0-9!@#$%^&*()_+\-=\[\]{};\':"\\|,.<>\/?]*$/', $value)) {
            $this->validationObject->addError($field,  "This field requires at least one special character.");
        }
    }

    protected function noWhitespaces(mixed $value, string $field): void {
        if (preg_match("/\s/", $value)) {
            $this->validationObject->addError($field,  "This field must not contain any whitespaces.");
        }
    }

    protected function email(mixed $value, string $field): void {
        if (!filter_var($value, FILTER_VALIDATE_EMAIL)) {
            $this->validationObject->addError($field,  "This field must be a valid email address.");
        }
    }

    protected function matches(mixed $value, string $field, mixed $compareValue): void {
        if ($value !== $compareValue) {
            $this->validationObject->addError($field,  "This field must match the compared field.");
        }
    }
}